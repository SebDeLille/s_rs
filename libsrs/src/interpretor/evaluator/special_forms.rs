...
/// Handles `(set! <variable> <expr>)` (R7RS section 4.1.6). Evaluates
/// `<expr>` and rebinds the existing binding for `<variable>` in place,
/// searching the current environment and, if not found there, each
/// enclosing scope in turn (e.g. a binding introduced by `let`). Signals an
/// error if `<variable>` is unbound anywhere in the chain. Returns
    ...
    match args {
        [name, value_expr] => {
            ...
            if env.set(&name, value) {
                ...
                err(EvalErrorKind::UnboundVariable(name))
            }
        }
        [] | [_] => err(EvalErrorKind::NotEnoughArguments),
        _ => err(EvalErrorKind::TooManyArguments),
